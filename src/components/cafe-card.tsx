  ...
  return (
    <article className="group animate-fade-in overflow-hidden rounded-lg border border-border bg-card transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <Button variant="secondary" size="icon" aria-label={saved ? "Remove from saved" : "Save cafe"} onClick={() => toggleSaved(cafe.id)} className="absolute right-3 top-3 bg-background/90 active:scale-90">
      ...
      <div className="p-5">
        ...
        <div className="mt-5 border-y border-border py-3">
          <div className="flex items-center justify-between text-xs"><span>{percentage}% occupied</span><span className="font-semibold text-success-foreground">{seats} seats free</span></div>
          ...
          <div className="mt-2 flex items-center gap-1 text-xs text-muted-foreground"><Volume2 className="size-3.5" />{cafe.noise} noise level</div>
        </div>
        <div className="mt-4 flex flex-wrap gap-2">
          {cafe.facilities.slice(0, 3).map((tag) => <span key={tag} className="rounded-full bg-secondary px-3 py-1 text-xs text-secondary-foreground">{tag}</span>)}
        </div>
        <Button asChild className="mt-5 w-full"><Link to="/cafes/$slug" params={{ slug: cafe.slug }}>See details</Link></Button>
      </div>
    </article>
  );
}